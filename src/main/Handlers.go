...
func scanImage(server *Server, sessionToken *SessionToken, values url.Values,
	...
	if scanProviderName == "clair" {
		...
		sudo docker run -i -t -p 6060:6060 quay.io/coreos/clair:latest --db-type=bolt --db-path=/db/database
		*/
		...
		return NewFailureDesc("Unsupported scan provider: " + scanProviderName)
